...
%%{init: {
  ...
  'themeVariables': {
    'background': '#15171c',
    'primaryColor': '#1e222b',
    'primaryTextColor': '#e3e6ed',
    'primaryBorderColor': '#3f4654',
    'lineColor': '#8f9cae',
    'secondaryColor': '#282d37',
...
graph TD
    %% 節點樣式定義
    classDef startEnd fill:#d14949,stroke:#e66a6a,stroke-width:2px,color:#fff;
    classDef apiNode fill:#3b5998,stroke:#4e69a2,stroke-width:2px,color:#fff;
    classDef preNode fill:#2d3748,stroke:#4a5568,stroke-width:2px,color:#e2e8f0;
    classDef coreAi fill:#1a202c,stroke:#718096,stroke-width:2px,stroke-dasharray: 5 5,color:#e2e8f0;
    classDef stepNode fill:#2d3748,stroke:#4a5568,stroke-width:1px,color:#e2e8f0;
    classDef guardNode fill:#2b6cb0,stroke:#3182ce,stroke-width:2px,color:#fff;
    classDef decision fill:#dd6b20,stroke:#ed8936,stroke-width:2px,color:#fff;
    classDef saveNode fill:#2c5282,stroke:#3182ce,stroke-width:2px,color:#fff;

    %% 流程線段
    User([👤 使用者上傳相片]) -->|原始二進位圖檔| API[🚀 FastAPI: POST /api/analyze]
    API --> M1[⚙️ 模組 1: 影像前置處理 image_utils]
    
    %% 核心 AI 處理管線 (M2, M3, M4)
    subgraph AI_Pipeline [🧠 核心 AI 模組化處理管線]
        M1 -->|StandardizedImage| M2[📐 模組 2: 臉部幾何定位器 mediapipe_utils]
        ...
        %% M3 內部對每個區域跑所有模型
        subgraph M3_Internal [🧬 模組 3 內部: 區域全病灶掃描]
            M3 --> Crop1[左臉頰 Crop] --> Scan1[🔄 執行所有疾病模型 <br> 黑斑/白斑/皺紋/胎記...]
            M3 --> Crop2[右臉頰 Crop] --> Scan2[🔄 執行所有疾病模型 <br> 黑斑/白斑/皺紋/胎記...]
            M3 --> Crop3[其餘 3 區 Crops] --> Scan3[🔄 執行所有疾病模型 <br> 黑斑/白斑/皺紋/胎記...]
        end
    end
    ...
    %% M4 內部跨區域校正
    subgraph M4_Internal [🛡️ 模組 4 內部: 跨區域校正防禦機制]
        M4 --> Step1{該區域乾淨皮膚 < 20% ?}
        Step1 -->|No, 正常計算| Normal[🧮 計算該區中位數 Median RGB]
        Step1 -->|Yes, 觸發異常| Symmetrical[🔄 借用對側/全局健康膚色]
        Normal --> Step2[📊 產出 Skin Color Report]
    ...
    %% M5 VLM 決策
    M2 -->|Crops 影像| M5[👁️ 模組 5: VLM 決策引擎 vision]
    Scan1 & Scan2 & Scan3 -->|分區病灶面積 Context| M5
    Step2 -->|健康膚色基準| M5
    
    subgraph M5_Internal [✍️ 模組 5 內部: LLM 脈絡提示注入]
        M5 --> Prompt[📝 注入 Bias Hint 備註: <br> 'Left cheek baseline skin tone biased due to 99% Vitiligo']
        Prompt --> Claude[🤖 Claude Vision 綜合評估]
    end
    ...
    subgraph Decision_Engine [⚖️ 後端決策與安全攔截]
        FinalReport --> Triage{⚠️ medical_alert: 醫療警告檢查 <br> melanoma_suspected >= 0.85 ?}
        Triage -->|Yes, 觸發警報| Block[🚫 強制封鎖美妝推薦 <br> 清空推薦清單 + 標記紅字警告]
        Triage -->|No, 正常| RecOK[🛍️ recommendations: 尋找 44 筆粉底 <br> 附加遮瑕/校色技巧說明]
    end
    ...
    %% 節點樣式指派
    class User,Response startEnd;
    class API apiNode;
    class M1 preNode;
    class M2,M3,M4,M5 stepNode;
    class Crop1,Crop2,Crop3,Scan1,Scan2,Scan3 coreAi;
    class Step1,Symmetrical guardNode;
    class Triage,Block,RecOK decision;
    class SaveDB saveNode;