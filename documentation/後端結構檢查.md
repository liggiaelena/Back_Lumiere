...
後端專案採用 **FastAPI** 作為 Web 框架，整合 **MediaPipe BlazeFace** (人臉定位與放大裁剪)、**BiSeNet** (19分類用於臉部語意分區裁剪；14分類用於整體去瑕疵健康膚色估計)、**SegFormer** (多個獨立專家模型，執行黑斑、皺紋等膚況多標籤分割) 以及 **OpenAI Vision** (分區細緻膚質分析) 構成混合式 AI 分析管線。
...
* **服務啟動、路由與部署 (Server, Routes & Deployment)**
  ...
  * **設定管理器：** [config.py](file:///l:/Lumiere/Back_Lumiere/dev/app/config.py)
    * *職責：* 使用 `pydantic-settings` 讀取根目錄的 `.env`。包含自動辨識 LLM 提供者屬性 (`provider`)：優先使用 OpenAI 關鍵字。
...
* **分析管線與組件 (Analysis Pipeline & Utils)**
  * **核心管線協調器：** [pipeline.py](file:///l:/Lumiere/Back_Lumiere/dev/app/pipeline.py)
    * *職責：* 串接分析流程。主要邏輯如下：
      ...
      6. 並行發送 5 個區域的影像給 OpenAI Vision 進行細緻膚質預測，傳入 `condition_map` 作為背景 Context。
  ...
  * **LLM 膚質預測 (OpenAI Vision)：** [vision.py](file:///l:/Lumiere/Back_Lumiere/dev/app/vision.py)
    * *職責：* 組裝 analysis Prompt (融入 SegFormer 檢測結果作為 Context)，傳送 Base64 裁剪影像給 `gpt-5.6-luna`。Prompt 強制要求 OpenAI 必須比對視覺特徵，不能盲從 SegFormer Context，並一次回傳包含多國語系 (en, tw, zh, pt, fr, tr) 的 `notas` 字典。內建 Fallback 機制，當 OpenAI 呼叫失敗時回傳預設正常結構，避免系統崩潰。
...
### (2) BiSeNet 雙分割模型應用
* **19分類臉部解析 (Stage 3)**：使用 `79999_iter.pth` 模型。精確畫出額頭、臉頰、鼻子和下巴的精細物理邊界，為 OpenAI Vision 提供排除背景與頭髮後的去雜訊影像。
...
* 若 OpenAI Vision 在分析 5 大區域時，有 **2 個以上區域** 獨立回傳具有 `mancha`/`spot`/`mancha_solar` 等色素沉著瑕疵時，則觸發「對側/多區域互證」。
...
* **Individual Typology Angle (ITA) 評級**：
  ...
  並根據生物學界建立的角度範圍對應 Fitzpatrick 1~6 級。這比單純的 RGB 歐氏距離更能抵抗光照波動與面部泛紅的干擾。當無有效 HEX 時，Fallback 到 OpenAI 區域投票。
...
### (5) OpenAI Vision 區域分析與語系支援
* **防盲從設計**：OpenAI 接收分區影像與 SegFormer Context 進行分析，但必須依視覺特徵進行獨立評估，不得盲從 SegFormer 的預測。
* **語系 notes 字典**：OpenAI 被指示一次回傳多語系 notes 的 dict。
...
| `fitzpatrick_source` | `VARCHAR` | `NULLABLE` | 評級來源標記：`bisenet` (精密計算) 或 `openai` (LLM 票選) |
